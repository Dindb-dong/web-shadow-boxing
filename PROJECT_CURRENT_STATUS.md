...
Step 3 웹게임 MVP는 `Vite + TypeScript + Three.js + Docker` 기반 브라우저 클라이언트로 동작하며, 최근 UI 개선으로 `사용자 MediaPipe 포즈 오버레이`, `상반신 중심의 실제 GLB 아바타`, `좌/우 위빙 및 좌/우 더킹 기반 복싱 카운터 패턴`이 반영되었다. 실제 학습 모델은 아직 연결되지 않았으며, 팀원이 공유한 출력 스펙을 그대로 따르는 mock predictor가 기본 추론 엔진으로 들어가 있다. 이번 세션에서는 상단 중앙 AI HP 바를 추가하고, 회피 실패 시 AI HP가 감소하도록 연결했으며, 카운터 애니메이션도 몸통 회전과 반대손 가드를 포함하는 더 큰 공격 모션으로 강화했다. 오버레이는 전체 스켈레톤이 사용자 기준으로 정확히 반대라는 확인을 바탕으로, partial swap 로직을 제거하고 mirrored webcam preview에 맞춘 전체 좌우반전 렌더링으로 되돌렸다. 동시에 MediaPipe 입력 품질도 `full` 모델과 `720p / 60fps` 요청으로 상향했고, 손 포인트는 wrist 외에 thumb/index/pinky까지 표시하도록 확장했다. 또한 우측 상단에 게임 버전 뱃지를 추가하고, 현재 기준 버전을 `1.0.1`로 올렸다.
...
16. HUD 우측 상단에는 현재 게임 버전이 항상 보이며, 이번 기준 버전은 `v1.0.1`이다.
...
- 게임 버전은 `package.json`과 `src/version.ts`에서 `1.0.1`로 맞췄고, 앞으로 사소한 수정은 patch, 큰 수정은 minor를 올리는 규칙으로 관리한다.
